...
productos.forEach((item) => {
  ...
  espacioProductos.innerHTML = `<p class="nombreProducto">${item.prenda} ${item.equipo}</p>
...
const verCarrito = (producto) => {
  ...
  espacioCarrito.innerHTML = `
        <li class="listaCarrito">
            ...
            <ul class="caracteristica" id= eliminar${carrito.indexOf(
              producto
            )}><button>Eliminar</button></ul>
        </li>
        `;
  ...
  let botonEliminar = document.getElementById(
    `eliminar${carrito.indexOf(producto)}`
  );
